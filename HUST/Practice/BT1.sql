...
SELECT S.MASP, S.TENSP 
	FROM SANPHAM AS S 
	WHERE S.NUOCSX = 'Trung Quoc' 
	AND (S.GIA BETWEEN 30000 AND 40000)
GO

--Câu 5:
SELECT S.MASP, S.TENSP
	FROM SANPHAM AS S 
	WHERE (S.NUOCSX = 'Thai Lan'OR S.NUOCSX ='Trung Quoc')
	AND (S.GIA BETWEEN 30000 AND 40000)
GO
...
SELECT H.SOHD, H.TRIGIA 
	FROM HOADON AS H
	WHERE H.NGHD = '20070101' OR H.NGHD = '20070102'
GO

-- Câu 7:
SELECT *
	FROM HOADON AS H
	WHERE MONTH(H.NGHD) = 1 AND YEAR(H.NGHD) = 2007
	ORDER BY H.TRIGIA DESC
GO

-- Câu 8:
SELECT K.MAKH, K.HOTEN, H.MAKH FROM KHACHHANG AS K
...
SELECT H.SOHD, H.TRIGIA 
	FROM HOADON AS H
	WHERE H.MANV = (
		SELECT N.MANV FROM NHANVIEN AS N
...
-- Câu 10:
SELECT S.MASP, S.TENSP 
	FROM SANPHAM AS S INNER JOIN 
	CTHD AS C ON S.MASP = C.MASP
	INNER JOIN HOADON AS H 
	ON C.SOHD = H.SOHD
	WHERE (YEAR(H.NGHD) = 2006 AND MONTH(H.NGHD) = 10)
	AND H.MAKH = (
		SELECT K.MAKH FROM KHACHHANG AS K 
